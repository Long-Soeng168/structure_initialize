        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-4 py-3">{{__('messages.category')}}</th>
                    <th scope="col" class="px-4 py-3">{{__('messages.file')}}</th>
            ...
            <tbody>
                @forelse ($items as $item)
                    <tr wire:key='{{ $item->id }}'
                            ...
                            @if ($item->image)
                                <a href="{{ asset('assets/icons/audio_placeholder.png') }}" class="glightbox">
                                    <img src="{{ asset('assets/images/audios/thumb/' . $item->image) }}"
                                        ...
                                        class="object-cover border rounded-sm h-10 mr-3 aspect-[16/9]">
                                </a>
                            @else
                                <a href="{{ asset('assets/icons/audio_placeholder.png') }}" class="glightbox">
                                    <img src="{{ asset('assets/icons/audio_placeholder.png') }}"
                                        alt="iMac Front Image"
                                        class="object-contain border p-1 rounded-sm h-10 mr-3 aspect-[16/9]">
                                </a>
                            @endif
                        ...
                        <x-table-data
                            value="{{ $item->audioCategory?->name ? $item->audioCategory?->name : 'N/A' }}" />
                        <x-table-data>
                            @if ($item->file)
                                <span
                            ...
                            <div class="flex items-start justify-center gap-3">
                                @can('update audio')
                                    <div class="pb-1" x-data="{ tooltip: false }">
                                        <!-- Modal toggle -->
                                        <a href="{{ url('admin/audios_images/' . $item->id) }}"
                                ...
                                @can('view audio')
                                    <div class="pb-1" x-data="{ tooltip: false }">
                                        <!-- Modal toggle -->
                                        <a href="{{ url('audios/' . $item->id) }}" @mouseenter="tooltip = true"
                                ...
                                @can('update audio')
                                    <div class="pb-1" x-data="{ tooltip: false }">
                                        <!-- Modal toggle -->
                                        <a href="{{ url('admin/audios/' . $item->id . '/edit') }}"
