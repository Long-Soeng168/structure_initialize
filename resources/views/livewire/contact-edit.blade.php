<div>
    ...
    <form class="w-full" action="{{ route('admin.items.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div>
            ...
            <x-text-input id="iframe" class="block w-full mt-1" type="text" name="iframe"
                wire:model='iframe' required autofocus placeholder="Map Iframe" />
            <x-input-error :messages="$errors->get('iframe')" class="mt-2" />
        </div>
        ...
        <div class="mb-5" wire:ignore>
            <x-input-label for="contact_info" :value="__('messages.contact')" />
            <textarea id="contact_info" name="contact_info" wire:model="contact_info"></textarea>
        </div>
...
@script
    <script>
        ...
        $(document).ready(function() {
            const editor = CKEDITOR.replace('contact_info', options);
            editor.on('change', function(event) {
                console.log(event.editor.getData())
