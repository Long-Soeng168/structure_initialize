<div>
    ...
    <div
                ...
                <div>
                    <button id="filterDropdownButton" data-dropdown-toggle="filterDropdown"
                        ...
                        <p class="w-full text-left line-clamp-1">
                            {{ $selectedCategory ? $selectedCategory->name : 'Positions' }}
                        </p>
                            ...
                            @foreach ($categories as $category)
                                <li class="flex items-center">
                                    <button wire:click.prevent='setFilter("{{ $category->name }}")'>
                                        <p
                                            class="ml-2 uppercase text-sm font-medium text-gray-900 dark:text-gray-100 text-left hover:underline {{ $category->id == $filter ? 'underline' : '' }}">
                                            {{ $category->name }}
                                        </p>
        ...
        <div
            ...
            @can('create page')
                <x-primary-button href="{{ route('admin.gallery.create') }}">
                ...
                @forelse ($items as $item)
                    <tr wire:key='{{ $item->id }}'
                        ...
                        <th scope="row"
                            ...
                            <a href="{{ asset('assets/images/gallery/' . $item->image) }}" class="glightbox">
                                <img src="{{ asset('assets/images/gallery/thumb/' . $item->image) }}"
                                    alt="iMac Front Image" class="object-contain h-10 mr-3 aspect-[16/9]">
                            </a>
                        </th>
                        <x-table-data value="{{ $item->name }}" />
                        <x-table-data class="text-center uppercase" value="{{ $item->position }}" />
                            ...
                            <div class="flex items-start justify-center gap-3">
                                ...
                                @can('view page')
                                    <div class="pb-1" x-data="{ tooltip: false }">
                                        <!-- Modal toggle -->
                                        <a href="{{ url('gallery/' . $item->id) }}" @mouseenter="tooltip = true"
